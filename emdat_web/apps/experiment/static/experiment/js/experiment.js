...
var createSummary,loadSummaryTracker, loadSummaryValidity, getListParticipants, loadWarningMessage;
var loadWizard;
var saveParameters;

// Rules for the wizard form
var allRules = {
    // Tobii
    extra_header_lines:{
        required: true,
...
// Custom rule: Value muste be greater or equal
...
// Custom rule: Valid list of numbers
...
/**
 * Load checkboxes.
 * plugin - Bootstrap-checkbox (https://vsn4ik.github.io/bootstrap-checkbox/)
 */
...
/**
 * Load dual list boxes for features.
 * plugin - Bootstrap Dual Listbox (https://github.com/istvan-ujjmeszaros/bootstrap-duallistbox)
 */
loadDualListBox = function(){
    $(idGazeList+', '+idAOIList+', '+idPupilList+', '+idDistanceList+', '+idEventsList).bootstrapDualListbox({
...
/**
 * Show/hide fields for the eye tracker selected.
 */
...
/**
 * Create all sections for the summary tab.
 */
createSummary = function(){
    loadSummaryTracker();
    loadSummaryFeatures();
    loadSummaryValidity();
    participantsList = getListParticipants();
    loadWarningMessage(participantsList);
};

/**
 * Create features section for summary
 */
loadSummaryFeatures = function(){
     // Clear content
    $('#tbody_gaze_features').empty();
    ...
    // Append features
    $("#gaze > option:selected").each(function() {
        $('#tbody_gaze_features').append('<tr><td>' + this.text + '<td></tr>');
    });
...
/**
 * Create tracker section for summary
 */
...
/**
 * Create validity section for summary
 */
...
loadWarningMessage = function(participants){
    ...
    $.each( participants, function( key, value ) {
        ...
        fileList.append('<li>P'+ value + '-All-Data.tsv</li>');
        fileList.append('<li>P' + value + '-Event-Data.tsv</li>');
        fileList.append('<li>P' + value + '-Fixation-Data.tsv</li>');
        fileList.append('<li>P' + value + '.seg</li>');
    });
 ...
 * plugin - Twitter Bootstrap Wizard (https://github.com/VinceG/twitter-bootstrap-wizard)
 */
loadWizard = function(){
    wizard = $(idWizard);
...
/**
 * Change button status and submit form
 */
